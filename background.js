...
function saveconfig() {
 Q.ajaxc( { command: "http://www.baidu.com/?mod=user&action=do-save-config&inajax=true",
...
function ajax( json ) 
{
  ...
  //http_call.open("GET", "http://www.baidu.com/?mod=user&action=status&inajax=true&rnd="+Math.floor(+new Date/1E7), true);
  http_call.send(null);
}
...
setTimeout(function() {
  ...
  ajax( { command: "http://www.baidu.com/filter-rules.json",
    method: "GET",
...
function wayixia_logout( fn ) {
  ajax_execute( { command: "http://www.baidu.com/?mod=user&action=logout",
    method: "GET",
    oncomplete : function( res ) {
      // Clear user data
      wayixia.nickname = "";
...
function wayixia_statics_images( item, pageurl ) {
  ...
  Q.ajaxc( { command: "http://www.baidu.com/?mod=statics&action=image&inajax=true",
    queue: true,
...
function create_upgrade_page() {  
  var manager_url = "http://baidu.com//chrome/#v."+chrome.runtime.getManifest().version;
...
chrome.extension.onMessage.addListener( function( o ) {
  ...
  case "userstatus":
    //if( wayixia.nickname == ""  ) {
      ajax( { command: "http://www.baidu.com/?mod=user&action=status&withalbums=true&inajax=true",
        method: "GET",
